/**
 * Call `requirement()` inside a Jest test body to link the test to requirement IDs.
 *
...
export function requirement(ids: string | string[]): void {
  ...
  if (!testName) {
    throw new Error('requirement() must be called inside a Jest test body');
  }
